...
bool UdpSocket::bind(uint16_t port) {
    // close() leaves us without a socket; make a fresh one so returning to the
    // title screen and joining again works.
    if (!open_ && !openSocket()) return false;
    ...
    // Non-blocking, so the game loop never stalls on the network.
#ifdef _WIN32
    u_long mode = 1;
...
std::string UdpSocket::getLocalIPAddress() {
    ...
    if (probeOpen) {
        ...
        // connect() on UDP sends nothing - it just makes the OS pick the
        // interface it would route through, which getsockname then reports.
